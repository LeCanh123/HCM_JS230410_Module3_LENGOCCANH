...
let [el,setEl]=useState({
  actualPrice:"1", type:"1", image:"1", price:"1", title:"1", discount:"1"
})


  useEffect(async ()=>{
    try{
      let singleItem=await apis.getSinglecard(id)
      console.log(singleItem.data.data);

      if(singleItem.data.status){
        setEl(singleItem.data.data.data)
      }else{
        toast({
          title: "Hệ thống bận thử lại sau!1",
          description: "Err",
          status: "error",
          duration: 2000,
          isClosable: true,
  ...
  const handleAdd = async() => {
    ...
    console.log("add1111111111111111111111",add1);
if(add1.status){
          toast({
                  title: "Success",
            ...
            >
              <VStack spacing={{ base: 4, sm: 6 }}>
                <Text
                  ...
                  fontWeight={"300"}
                >
                  Our men's jeans are crafted with utmost care and attention to detail,
                </Text>
                <Text fontSize={"lg"}>
